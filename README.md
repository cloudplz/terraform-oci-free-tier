...
| [oci_core_network_security_group_security_rule.compute_ingress_tcp](https://registry.terraform.io/providers/oracle/oci/latest/docs/resources/core_network_security_group_security_rule) | resource |
...
| <a name="input_compute_ingress_tcp_rules"></a> [compute\_ingress\_tcp\_rules](#input\_compute\_ingress\_tcp\_rules) | Additional TCP ingress rules for the compute network security group. Use this to expose explicit services such as HTTPS while keeping SSH controlled separately. | <pre>map(object({<br/>    port        = number<br/>    source_cidr = string<br/>    description = optional(string)<br/>  }))</pre> | `{}` | no |
...
| <a name="input_user_data"></a> [user\_data](#input\_user\_data) | Optional cloud-init or shell script user data. The module base64-encodes it before sending it to OCI. When enable\_keepalive is true, this is composed with the keepalive script via multipart MIME. | `string` | `null` | no |
...
| <a name="output_compute_network_security_group_id"></a> [compute\_network\_security\_group\_id](#output\_compute\_network\_security\_group\_id) | OCID of the compute network security group attached to compute instance VNICs. |
